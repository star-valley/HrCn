...
int main()
{
	...
	/*边缘检测*/
	IplImage* edge = EdgeDtc(bnr);
	cvSaveImage(path_edge.c_str(), edge);

	/*轮廓提取*/
	IplImage* imcontour = cvCreateImage(cvGetSize(edge), 8, 1);
	cvZero(imcontour);
	CvSeq* contour = ContourExtr(edge, imcontour);

	/*凸包描述*/
	CvPoint* Dweb = new CvPoint;
	IplImage* imdefects = cvCreateImage(cvGetSize(edge), 8, 3);
	cvZero(imdefects);
